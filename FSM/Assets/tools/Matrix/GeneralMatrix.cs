...
namespace DotNetMatrix
{
	...
	public class GeneralMatrix : System.ICloneable, System.Runtime.Serialization.ISerializable, System.IDisposable
	{
		...
		virtual public double this[int iRow, int iCol]      // Access this matrix as a 2D array
		{
			get { return A[iRow][iCol]; }
			set { A[iRow][iCol] = value; }
		}
		...
		public virtual GeneralMatrix PseudoInverse()
		{
			SingularValueDecomposition svd = new SingularValueDecomposition(this);
			double pinvtoler = Math.Pow(10, -6);
			double[] m_sigma = svd.SingularValues;
			GeneralMatrix m_sigma_inv = new GeneralMatrix(this.ColumnDimension, this.RowDimension);
			for (int i=0; i<m_sigma.Length; ++i)
			{
				if (m_sigma[i] > pinvtoler)
					m_sigma_inv[i,i] = 1.0 / m_sigma[i];
			}
			return svd.GetV() * m_sigma_inv * svd.GetU().Transpose();
